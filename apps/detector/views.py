...
def draw_texts(result_image, line, c1, cv2, color, labels, label):
    ...
    font = max(line - 1, 1)
    t_size = cv2.getTextSize(display_txt, 0, fontScale=line / 3,
                             thickness=font)[0]
...
def exec_detect(target_image_path):
    ...
    cv2.imwrite(detected_image_file_path, cv2.cvtColor(result_image,
                                                       cv2.COLOR_RGB2BGR))
...
def save_detected_image_tags(user_image, tags, detected_image_file_name):
    ...
    for tag in tags:
        user_image_tag = UserImageTag(user_image_id=user_image.id,
                                      tag_name=tag)
...
def detect(image_id):
    ...
    # 物体検知対象の画像パスを取得する
    target_image_path = Path(current_app.config["UPLOAD_FOLDER"],
